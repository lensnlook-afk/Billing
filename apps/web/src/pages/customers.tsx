      ...
      <div className="modal modal-sm" onClick={e => e.stopPropagation()}>
        ...
        <form onSubmit={submit} className="modal-body">
          <div className="form-group">
            ...
            <input value={form.name} onChange={set('name')} placeholder="e.g. Rahul Kumar" required autoFocus />
          </div>
          <div className="form-row">
            <div className="form-group">
              <label>Mobile</label>
              <input value={form.mobile} onChange={set('mobile')} placeholder="9876543210" inputMode="tel" />
            </div>
            <div className="form-group">
              <label>Email</label>
...
function CustomerDetailModal({ customer, onClose, onEdit }: { customer: CustomerDetail; onClose: () => void; onEdit: () => void }) {
    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal" onClick={e => e.stopPropagation()}>
        <div className="modal-header">
          <div>
            <h3>{customer.name}</h3>
            <p style={{ fontSize: 12, color: '#64748b', margin: '2px 0 0' }}>C-{customer.customer_no}</p>
          </div>
        ...
        <div className="modal-body">
          ...
          <div className="cust-info-grid">
            {customer.mobile && <div className="cust-info-item"><span>Mobile</span><strong>{customer.mobile}</strong></div>}
          ...
          {customer.notes && <p style={{ fontSize: 13, color: '#64748b', background: '#f8fafc', borderRadius: 8, padding: '10px 12px' }}>{customer.notes}</p>}

          {/* Purchase history */}
          {customer.invoices.length > 0 && (
            <div>
              <p style={{ fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '.6px', color: '#94a3b8', marginBottom: 8 }}>Purchase History</p>
    ...
    <div className="inv-page">
      ...
      <div className="inv-kpis">
        <div className="inv-kpi">
          ...
          <div className="inv-kpi-value">{totalCustomers}</div>
        </div>
        <div className="inv-kpi">
        ...
        <div className="inv-kpi inv-kpi-accent">
          <div className="inv-kpi-label">Results Shown</div>
      ...
      {loading ? (
        ...
        <div className="inv-empty">
          ...
          <button className="btn-primary" style={{ marginTop: 12 }} onClick={() => setShowAdd(true)}>Add first customer</button>
        </div>
      ) : (
        <>
          {/* Desktop table */}
              ...
              <tbody>
                {customers.map(c => (
                  <tr key={c.id} style={{ cursor: 'pointer' }} onClick={() => openDetail(c.id)}>
                    <td style={{ color: '#64748b', fontSize: 12 }}>C-{c.customer_no}</td>
                    <td><div className="inv-product-name">{c.name}</div></td>
                    <td>{c.mobile ?? '—'}</td>
                    <td style={{ color: '#64748b', fontSize: 12 }}>{c.email ?? '—'}</td>
                    <td style={{ color: '#64748b', fontSize: 12 }}>{fmt(c.created_at)}</td>
                    <td><button className="inv-add-stock-btn" onClick={e => { e.stopPropagation(); openDetail(c.id); }}>View</button></td>
                  </tr>
                ))}
              </tbody>
            </table>
            ...
            {customers.map(c => (
              <div key={c.id} className="inv-card" onClick={() => openDetail(c.id)} style={{ cursor: 'pointer' }}>
                <div className="inv-card-top">
                  <div className="inv-card-name">{c.name}</div>
                  <button className="inv-add-stock-btn" onClick={e => { e.stopPropagation(); openDetail(c.id); }}>View</button>
                </div>
                <div className="inv-card-meta">
                  <span style={{ fontSize: 11, color: '#64748b' }}>C-{c.customer_no}</span>
                  {c.mobile && <span style={{ fontSize: 12, color: '#475569' }}>📱 {c.mobile}</span>}
